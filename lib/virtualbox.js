...
class Virtualbox {
  ...
  uploadTo(vmname, source, destination, callback) {
    this._logging.info('Uploading file to VM "%s"', vmname);
    var cmd = ['guestcontrol', vmname, '--username', 'Guest', 'copyto', source, destination];
    this.vboxmanage(cmd, callback);
  }

  downloadFrom(vmname, source, destionation, callback) {
    this._logging.info('Downloading file from VM "%s"', vmname);
    var cmd = ['guestcontrol', vmname, '--username', 'Guest', 'copyfrom', source, destionation];
    this.vboxmanage(cmd, callback);
  }
}
